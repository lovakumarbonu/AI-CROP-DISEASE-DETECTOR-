...
const ResultDisplay: React.FC<ResultDisplayProps> = ({ results, imagePreviewUrl, onReset }) => {
  ...
  const speakAnalysis = () => {
    ...
    utterance.onerror = (event) => {
        // User-initiated cancellations can fire an 'interrupted' or 'canceled' error.
        // We can safely ignore these as they are not unexpected application errors.
        if (event.error === 'interrupted' || event.error === 'canceled') {
            // Not a true error, just a confirmation of cancellation.
        } else {
              ...
              return (
                <div key={index} className="bg-slate-900/50 p-4 rounded-lg border-l-4" style={{ borderColor: color }}>
                    <div className="flex items-center gap-3">
                        ...
                        {!result.is_healthy && result.disease_info_url && (
                            <a
                                href={result.disease_info_url}
                                target="_blank"
                                rel="noopener noreferrer"
                                className="ml-auto flex items-center gap-1.5 text-xs font-semibold text-blue-400 bg-blue-900/50 px-2 py-1 rounded-full hover:bg-blue-900 hover:text-blue-300 transition-colors"
                                aria-label={`Learn more about ${result.disease_name}`}
                            >
                    ...
                    <div className="mt-4 space-y-4">
                        ...
                        {result.plant_name && (
                            <div>
                                <h3 className="text-md font-semibold text-green-400 flex items-center gap-2 mb-2"><LeafIcon />{translations.plantIdentification}</h3>
                                <p className="text-slate-200">{result.plant_name}</p>
                            </div>
                        )}
