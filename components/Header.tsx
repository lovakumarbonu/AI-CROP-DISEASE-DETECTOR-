...
import { ScanIcon, HistoryIcon, CalendarIcon } from './Icons';

interface HeaderProps {
    activeView: 'scanner' | 'history' | 'prevention';
    setActiveView: (view: 'scanner' | 'history' | 'prevention') => void;
}

const Header: React.FC<HeaderProps> = ({ activeView, setActiveView }) => {
  const { translations } = useLanguage();

  const navButtonClasses = (view: 'scanner' | 'history' | 'prevention') => 
    ...
    <header className="text-center mb-8 relative pb-16">
      ...
      <div className="mt-8 flex justify-center">
        <div className="flex space-x-2 bg-slate-800/80 border border-slate-700 p-1.5 rounded-lg">
            ...
            <button className={navButtonClasses('prevention')} onClick={() => setActiveView('prevention')}>
                <CalendarIcon />
                {translations.preventionTab}
            </button>
        </div>
      </div>
